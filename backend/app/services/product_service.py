...
def list_products(db: sqlite3.Connection, user_id: int) -> list[Product]:
    rows = db.execute(
        """
        SELECT id, name, quantity, category, bought, user_id
        FROM products
...
def get_product(db: sqlite3.Connection, product_id: int, user_id: int) -> Product:
    return product_from_row(get_product_or_404(db, product_id, user_id))


def create_product(db: sqlite3.Connection, product: ProductCreate, user_id: int) -> Product:
    cursor = db.execute(
        """
    ...
    db.commit()
    return get_product(db, cursor.lastrowid, user_id)
    ...
    if updates:
        db.execute(
            ...
            (
                ...
                int(updates.get("bought", existing_product["bought"])),
                product_id,
                user_id,
            ),
        )
...
def mark_product_as_bought(db: sqlite3.Connection, product_id: int, user_id: int) -> Product:
    get_product_or_404(db, product_id, user_id)
    db.execute(
        "UPDATE products SET bought = 1 WHERE id = ? AND user_id = ?",
        (product_id, user_id),
    )
    db.commit()
    return get_product(db, product_id, user_id)
...
def delete_product(db: sqlite3.Connection, product_id: int, user_id: int) -> None:
    get_product_or_404(db, product_id, user_id)
    db.execute(
